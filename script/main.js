window.addEventListener('load', () => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // Play the background music if confirmed
            document.querySelector('.song').play();
            animationTimeline();
        } else {
            // Proceed with the animation without playing music
            animationTimeline();
        }
    });
});

// Animation timeline for birthday greeting
const animationTimeline = () => {
    const textBoxChars = document.getElementsByClassName("hbd-chatbox")[0];
    const hbd = document.getElementsByClassName("wish-hbd")[0];

    // Split the text into individual span elements for animation
    ...
    // Restart animation on clicking replay button
